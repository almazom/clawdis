...
- Config: `TELEGRAM_BOT_TOKEN` env or `telegram.botToken` required; `telegram.requireMention`, `telegram.allowFrom`, `telegram.mediaMaxMb`, `telegram.proxy`, `telegram.webhookSecret`, `telegram.webhookUrl`, `telegram.webhookPath`, `telegram.multyPublish`, `telegram.multyNotify` supported.
...
```json5
{
  telegram: {
    ...
    multyPublish: true, // Publish step for /multy
    multyNotify: false // Notify step for /multy
  }
}
```
